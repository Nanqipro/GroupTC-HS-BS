...
/**
 * @brief 记录变换前的 id，用来生成 d_idMapArr,并分割 degree
 */
__global__ void record_id_and_part_graph_by_degree(int edge_count, int vertex_count, int *d_idArr, int *d_degreeArr);
...
void compute() {
    ...
    for (int k = 0; k < iteration; k++) {
        ...
        record_id_and_part_graph_by_degree<<<vertex_grid_size, block_size>>>(edge_count, vertex_count, d_idArr, d_degreeArr);
        HRR(cudaDeviceSynchronize());
