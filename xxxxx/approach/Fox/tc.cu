...
__global__ void binSearchKernel() {
    ...
    while (edgeID < c_edgeSize) {
        ...
        for (int i = 0; i < (dstAdjListLen + c_threadsPerEdge - 1) / c_threadsPerEdge; i++) {
            ...
            if (dstListIdx >= dstAdjListLen) continue;
    ...
    for (int i = 16; i >= 1; i /= 2) sum += __shfl_down(sum, i);
...
void rearrangeEdge(edge *Edges, unsigned int binaryStartPos, unsigned curSize, int edgesInABlock, int *adjLength) {
...
#pragma omp parallel for
    for (int i = 0; i < curSize / groupSize; i++) {
        ...
        secIndex = i / blockNum;
        memcpy(Edges + binaryStartPos + (firstIndex * groupPerBlock + secIndex) * groupSize, curEdges + i * groupSize, sizeof(edge) * groupSize);
    }
...
int main(int argc, const char *argv[]) {
    ...
    cout << "Number of nodes: " << nodeNum << ", number of edges: " << edgeNum << endl;
    ...
    //	cout << "workLoad is " << edgeWorkLoad[i] << ",and the src and dst degree are " << adjLength[Edges[i].src] << " " << adjLength[Edges[i].dst]
    //<< endl;
    ...
    cudaFree(d_edgeWL);
    //	for (int i = 0; i < edgeNum; i++) {
    //		cout << "workLoad is " << edgeWorkLoad[i] << ",and the src and dst degree are " << adjLength[Edges[i].src] << " " <<
    // adjLength[Edges[i].dst] << endl;
    //	}
    // mv csr to GPU
    ...
    for (int iter = 0; iter < iterator_count; iter++) {
        ...
        for (int p = 0; p < totalPass; p++) {  // for each big block divided by thrust sort
            ...
            for (int i = 1; i <= curBinB; i *= 2) {  // for each bin
                ...
                // cout << "start point workload " << edgeWorkLoad[binStartPos[i]] << ", end point workload " << edgeWorkLoad[binStartPos[i + 1] - 1]
                // << endl;
                ...
                // cout << " use time " << time*1000 << " ms." << endl << endl;

                totalTCTimeInMsec += (double)1000 * (tEnd - tStart) / CLOCKS_PER_SEC;
            }  // for each bin
            delete[] binStartPos;

        }  // for e|ach big block divided by thrust sort
           // cout << "kernel use time " << kernelUse << " s." << endl;
           // if (iter != 0) {
