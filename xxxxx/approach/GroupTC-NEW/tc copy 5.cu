...
grouptc_with_reduce(vertex_t *src_list, vertex_t *adj_list, index_t *beg_pos, int edge_count, int vertex_count, unsigned long long *GLOBAL_COUNT)
{
	...
	for (int i = bid * block_bucketnum; i < edge_count; i += gridDim.x * block_bucketnum)
	{
		...
		while (now < end)
		{
			...
			if (now < end)
			{
				int *arr = adj_list + sh_tb_start[now];
				int arr_len = sh_tb_len[now];
				int *search_arr = adj_list + sh_ele_start[now] + workid;
				for (int tempi = 0; tempi < 4 && tempi < neighbor_degree - workid; tempi++)
				{
					offset = last_now == now ? offset : 0;
					P_counter += bin_search(arr + offset, arr_len - offset, search_arr[tempi], offset);
				}
				last_now = now;
				// int len = sh_tb_len[now] - offset;
				// if (len > 8)
				// {
				// }
				// P_counter += bin_search(adj_list + sh_tb_start[now] , sh_tb_len[now], adj_list[sh_ele_start[now] + workid]);
			}
			workid += subwarp_size * 4;
		}
		__syncthreads();
	}
