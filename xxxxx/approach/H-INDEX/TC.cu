...
__device__ void kogge_sum(int *A, int len, int start)
{
...
__device__ int linear_search(int neighbor, int *partition1, int *bin_count, int bin, int BIN_OFFSET, int BIN_START, int BUCKETS)
{
...
struct arguments Triangle_count(int rank, char name[100], struct arguments args, int total_process, int n_threads, int n_blocks, int BUCKETS, int select_thread_group, int select_partition)
{
	...
	unsigned long long triangleCount = counter[0];
...
int main(int argc, char *argv[])
{
	...
	// printf("%s,%d,%d,%lld,%fs,%fTEPS\n", argv[1], args.vertices, args.edge_count, args.count, args.time, (args.edge_count / args.time / 1000000000));
	return 0;
}
