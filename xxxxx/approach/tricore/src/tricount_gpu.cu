...
uint32_t cal_part_num(const uint64_t *edges, uint64_t edge_count, uint32_t node_count) {
    ...
    while (true) {
        ...
        uint64_t tri_use_mem = part_edge_count * sizeof(uint64_t) * 2 * 115 / 100 + (part_node_count + 1) * sizeof(uint32_t) * 2 +
                               numBlocks * BLOCKSIZE * sizeof(uint64_t);
...
__global__ void node_index_construct_kernel(const uint32_t *edges_first, uint64_t edge_count, uint32_t *node_index, uint32_t node_count,
                                            uint32_t start_node) {
    auto from = blockDim.x * blockIdx.x + threadIdx.x;
    auto step = gridDim.x * blockDim.x;
    for (uint64_t i = from; i <= edge_count; i += step) {
...
struct is_self_loop : public thrust::unary_function<uint64_t, bool> {
    __host__ __device__ bool operator()(uint64_t x) { return FIRST(x) == SECOND(x); }
};
...
__global__ void warp_binary_kernel(const uint32_t *__restrict__ edge_m, const uint32_t *__restrict__ node_index_m, uint32_t edge_m_count,
                                   uint32_t *__restrict__ adj_m, uint32_t start_node_n, const uint32_t *__restrict__ node_index_n,
                                   uint32_t node_index_n_count, uint32_t *__restrict__ adj_n, uint64_t *results) {
    // phase 1, partition

    if (threadIdx.x == 0 && blockIdx.x == 0) {
        printf("%d %d %d %d\n", edge_m_count, start_node_n, node_index_n_count);
    }
    uint64_t count = 0;
...
__global__ void warp_intersection_kernel(const uint32_t *__restrict__ edge_m, const uint32_t *__restrict__ node_index_m, uint32_t edge_m_count,
                                         uint32_t *__restrict__ adj_m, uint32_t start_node_n, const uint32_t *__restrict__ node_index_n,
                                         uint32_t node_index_n_count, uint32_t *__restrict__ adj_n, uint64_t *results) {
    // phase 1, partition
    uint64_t count = 0;
...
uint64_t tricount_gpu(uint64_t part_num, uint32_t *adj, const uint64_t *adj_count, const uint32_t *node_split, uint32_t **node_index,
    ...
    for (int iter = 0; iter < iterator_count; iter++) {
        ...
        for (uint64_t m = 0; m < part_num; m++) {
            ...
            kernelUse += omp_get_wtime() - startKernel;
            CUDA_TRY(cudaDeviceSynchronize());
            for (uint64_t n = m; n < part_num; n++) {
                ...
                startKernel = omp_get_wtime();
                cudaProfilerStart();
                warp_binary_kernel<<<numBlocks, BLOCKSIZE>>>(edge_m_dev, node_index_m_dev, adj_count_m, adj_m_dev, start_node_n, node_index_n_dev,
                                                             node_index_n_count, adj_n_dev, dev_results);
                CUDA_TRY(cudaDeviceSynchronize());
                cudaProfilerStop();
                ...
                if (m != n) {
                    ...
                    //                log_info("tricount_gpu_edge_kernel start");
                    cudaProfilerStart();
                    warp_binary_kernel<<<numBlocks, BLOCKSIZE>>>(edge_n_dev, node_index_n_dev, adj_count_n, adj_n_dev, start_node_m, node_index_m_dev,
                                                                 node_index_m_count, adj_m_dev, dev_results);
                    CUDA_TRY(cudaDeviceSynchronize());
                    cudaProfilerStop();
                    //                log_info("tricount_gpu_edge_kernel end");
        ...
        ee = omp_get_wtime();
        kernelUse = ee - startKernel;
        total_kernel_use += kernelUse;
        if (iter == 0) {
            if (kernelUse > 0.1 && iterator_count != 1) {
                iterator_count = 10;
            }
        }
    }
    // printf("total %d\n", total/(iterator_count+1));
