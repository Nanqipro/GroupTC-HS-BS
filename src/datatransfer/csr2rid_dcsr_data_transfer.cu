...
void Csr2RidDcsrDataTransfer::transfer() {
    ...
    spdlog::info("CSR2RidDCSR graph transfer start, graph max degree is {}", max_degree);
    ...
    for (int k = 0; k < iterations; k++) {
        ...
        if (edge_count > constant_comm::kDataTransferMaxEdgeCount) {
            spdlog::info("Csr2RidDcsrDataTransfer's sorting requires more GPU space, so src_list and adj_list are transferred to CPU space.");
    ...
    spdlog::get("csr2rid_dcsr_file_logger")
        ->info("{0}\t{1}\t{2}\t{3:.6f}", "csr2rid_dcsr", d_graph.input_dir, iterations, (t_end - t_start) / iterations);
    ...
    spdlog::info("CSR2RidDCSR graph transfer finished, graph max degree is {}", max_degree);
