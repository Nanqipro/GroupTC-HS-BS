...
void tc::approach::GroupTC_HASH::gpu_run(INIReader &config, GPUGraph &gpu_graph, std::string key_space) {
    ...
    spdlog::get("GroupTC-HASH_file_logger")
        ->info("{0}\t{1}\t{2}\t{3}\t{4:.6f}\t{5}\t{6}\t{7}\t{8}\t{9}\t{10}", "GroupTC-Hash", gpu_graph.input_dir, counter[0], iteration_count, total_kernel_use / iteration_count, edge_count,vertex_count,warpfirstvertex, warpfirstedge, nocomputefirstvertex, nocomputefirstedge);
