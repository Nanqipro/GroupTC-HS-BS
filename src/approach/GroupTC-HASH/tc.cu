...
void tc::approach::GroupTC_HASH::gpu_run(INIReader &config, GPUGraph &gpu_graph, std::string key_space) {
    ...
    auto logger = spdlog::get("GroupTC-HASH_file_logger");
    if (logger) {
        logger->info("{0}\t{1}\t{2}\t{3}\t{4:.6f}", "GroupTC-HASH", gpu_graph.input_dir, counter[0], iteration_count, total_kernel_use / iteration_count);
    } else {
        spdlog::warn("Logger 'GroupTC-HASH_file_logger' is not initialized.");
    }
