...
void tc::approach::TriCore::gpu_run(INIReader& config, GPUGraph& gpu_graph, std::string key_space) {
    ...
    auto logger = spdlog::get("TriCore_file_logger");
    if (logger) {
        logger->info("{0}\t{1}\t{2}\t{3}\t{4:.6f}", "TriCore", gpu_graph.input_dir, count, iteration_count, total_kernel_use / iteration_count);
    } else {
        spdlog::warn("Logger 'TriCore_file_logger' is not initialized.");
    }
