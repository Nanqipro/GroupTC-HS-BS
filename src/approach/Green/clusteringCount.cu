  ...
  while(!found)
  {
    ...
    *outVCurr = diagonalId - *outUCurr;
    if(*outVCurr >= *outVMax)
    {
     ...
     uint comp1 = uNodes[*outUCurr] > vNodes[*outVCurr-1];
     uint comp2 = uNodes[*outUCurr-1] > vNodes[*outVCurr];
    if(comp1 && !comp2)
    {
      found = 1;
    }
...
{
   uint uBigger = (*uCurr > 0) && (*vCurr < vLength) &&
    (uNodes[*uCurr-1] == vNodes[*vCurr]);
   uint vBigger = (*vCurr > 0) && (*uCurr < uLength) &&
    ...
    const T number_blocks, const T shifter)
{
  ...
  for (T src = this_mp_start; src < this_mp_stop; src++)
  {
		...
		for(T iter=d_off[src]+adj_offset; iter<d_off[src+1]; iter+=number_blocks)
    {
      ...
      bool avoidCalc = (src == dest)||(srcLen < 2)||(destLen < 1) ;
      if(avoidCalc)
        continue;
...
template <typename T>
void kernelCall( uint numberBlocks,  uint numberThreads,
...
template <typename T>
float timedKernelCall( uint numberBlocks,  uint numberThreads,
    const T nv, T const * const __restrict__ vertex,
    T const * const __restrict__ d_off, T const * const __restrict__ d_ind,
    T * const __restrict__ outPutTriangles, const T threads_per_block,
    const T number_blocks, const T shifter)
...
template void kernelCall<int32_t>( uint numberBlocks,
     uint numberThreads, const int32_t nv,
...
template void kernelCall<int64_t>( uint numberBlocks,
     uint numberThreads, const int64_t nv,
...
template float timedKernelCall<int32_t>( uint numberBlocks,
     uint numberThreads, const int32_t nv,
    int32_t const * const __restrict__ d_off,
    int32_t const * const __restrict__ d_ind,
    int32_t * const __restrict__ outPutTriangles,
...
template float timedKernelCall<int64_t>( uint numberBlocks,
     uint numberThreads, const int64_t nv,
    int64_t const * const __restrict__ d_off,
    int64_t const * const __restrict__ d_ind,
    int64_t * const __restrict__ outPutTriangles,
