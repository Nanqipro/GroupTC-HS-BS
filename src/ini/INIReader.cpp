...
bool INIReader::GetBoolean(const string& section, const string& name, bool default_value) const {
    ...
    std::transform(valstr.begin(), valstr.end(), valstr.begin(), [](const unsigned char& ch) { return static_cast<unsigned char>(::tolower(ch)); });
...
bool INIReader::HasSection(const string& section) const {
    ...
    if (pos == _values.end()) return false;
...
string INIReader::MakeKey(const string& section, const string& name) {
    ...
    std::transform(key.begin(), key.end(), key.begin(), [](const unsigned char& ch) { return static_cast<unsigned char>(::tolower(ch)); });
    return key;
}
