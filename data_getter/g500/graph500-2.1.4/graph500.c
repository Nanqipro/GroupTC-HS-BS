...
int main(int argc, char **argv)
{
  ...
  */
  ...
  // 文件指针pd
  FILE *bin_file = NULL;
  // char *variable_value = getenv("G500_PATH");
  // if (variable_value != NULL)
  // {
  //   printf("The value of G500_PATH is: %s\n", variable_value);
  // }
  // else
  // {
  //   printf("G500_PATH does not exist.\n");
  // }
  // bin_file = fopen("cache.bin", "wb");

  // char str0[20] = {'s'};
  // char str1[20];
  // char str2[20];

  // sprintf(str1, "%d", SCALE);
  // strcat(str0, str1);
  // strcat(str0, "-e");
  // sprintf(str2, "%d", edgefactor);
  // strcat(str0, str2);
  // strcat(str0, ".bin");

  // bin_file = fopen(str0, "wb");
  // bin_file = fopen("cache.bin", "wb");
  // bin_file = fopen(variable_value, "wb");
  // fwrite(&IJ[0], sizeof(int64_t) * 2, nedge, bin_file);
  // fclose(bin_file);
  ...
  // {
  //   // 1-nedge
  //   if (IJ[i].v0 == 0 || IJ[i].v1 == 0 || IJ[i].v0 >= nedge || IJ[i].v1 >= nedge)
  //   {
  //     printf("xx");
  //     break;
  //   }
